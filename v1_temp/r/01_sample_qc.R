...
sampleqc2 = sampleqc %>% arrange(TITLE, FINAL_POP)
...
p1 = ggplot(sampleqc2, aes(x=1:nrow(sampleqc), y=CHIMERAS, colour=TITLE, shape=dropped_chimera)) + geom_point() + theme(legend.position = 'none') + geom_hline(yintercept=.05, linetype=2) + ylab('Chimera rate') + xlab('')

# , shape=keep
p2 = ggplot(sampleqc2, aes(x=1:nrow(sampleqc), y=callRate, colour=TITLE, shape=dropped_hardfilters)) + geom_point() + theme(legend.position = 'none') + geom_hline(yintercept=.95, linetype=2) + ylab('Call rate') + xlab('')

p3 = ggplot(sampleqc2, aes(x=1:nrow(sampleqc), y=dpMean, colour=TITLE, shape=dropped_DP)) + geom_point() + theme(legend.position = 'none') + geom_hline(yintercept=15, linetype=2) + ylab('Coverage') + xlab('')

p0 = ggplot(sampleqc2, aes(x=1:nrow(sampleqc), y=dpMean, colour=TITLE, shape=dropped_DP)) + geom_point() + theme(legend.text=element_text(size=7), legend.key.size=unit(4, 'mm'), legend.title = element_blank()) + guides(shape=F) + geom_hline(yintercept=15, linetype=2) + ylab('Coverage') + xlab('')

p4 = g_legend(p0)

pdf('plots/01_sample_qc_chimera_callrate_coverage.pdf')
...
ggplot(sampleqc3, aes(x=index, y=rTiTv, colour=FINAL_POP)) + geom_point() + geom_hline(aes(yintercept=median_rTiTv + 4*mad_rTiTv), linetype=2) + geom_hline(aes(yintercept=median_rTiTv - 4*mad_rTiTv), linetype=2) + ylab('Ti/Tv ratio') + xlab('') + facet_wrap(~ TITLE, scale='free_x')

ggsave('plots/01_sample_qc_titv.pdf')


ggplot(sampleqc3, aes(x=index, y=rHetHomVar, colour=FINAL_POP)) + geom_point() + geom_hline(aes(yintercept=median_rHetHomVar + 4*mad_rHetHomVar), linetype=2) + geom_hline(aes(yintercept=median_rHetHomVar - 4*mad_rHetHomVar), linetype=2) + ylab('Het/Hom ratio') + xlab('') + facet_wrap(~ TITLE, scale='free_x')

ggsave('plots/01_sample_qc_hethomvar.pdf')


ggplot(sampleqc3, aes(x=index, y=rInsertionDeletion, colour=FINAL_POP)) + geom_point() + geom_hline(aes(yintercept=median_rInsertionDeletion + 4*mad_rInsertionDeletion), linetype=2) + geom_hline(aes(yintercept=median_rInsertionDeletion - 4*mad_rInsertionDeletion), linetype=2) + ylab('Ins/Del ratio') + xlab('') + facet_wrap(~ TITLE, scale='free_x')
